...
public class ExampleEurekaGovernatedService {
	...
	private static LifecycleInjector init() throws Exception {
		...
		LifecycleInjector injector = InjectorBuilder.fromModules(new EurekaModule(), new ExampleServiceModule())
				.overrideWith(new AbstractModule() {
					@Override
					protected void configure() {
						DynamicPropertyFactory configInstance = com.netflix.config.DynamicPropertyFactory.getInstance();
						bind(DynamicPropertyFactory.class).toInstance(configInstance);
						// the default impl of EurekaInstanceConfig is CloudInstanceConfig, which we
						// only want in an AWS
						// environment. Here we override that by binding MyDataCenterInstanceConfig to
